...
* LOGO: kubernetes | openshift | eks | aks
* NEGATIVE: true | false
...
- [jhidalgo3/hello-kubernetes:3.0](https://hub.docker.com/r/jhidalgo3/hello-kubernetes/)
...
### Customise Message and change logo
...
### Specify Custom Port and invert colors
...
Bash
```bash
$ docker build --no-cache --build-arg IMAGE_VERSION="3.0" --build-arg IMAGE_CREATE_DATE="`date -u +"%Y-%m-%dT%H:%M:%SZ"`" --build-arg IMAGE_SOURCE_REVISION="`git rev-parse HEAD`" -f Dockerfile -t "jhidalgo3/hello-kubernetes:3.0" .
```

Powershell
```powershell
PS> docker build --no-cache --build-arg IMAGE_VERSION="3.0" --build-arg IMAGE_CREATE_DATE="$(Get-Date((Get-Date).ToUniversalTime()) -UFormat '%Y-%m-%dT%H:%M:%SZ')" --build-arg IMAGE_SOURCE_REVISION="$(git rev-parse HEAD)" -f Dockerfile -t "jhidalgo3/hello-kubernetes:3.0" .
```
